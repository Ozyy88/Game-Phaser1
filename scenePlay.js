var scenePlay = new Phaser.Class({
  ...
  update: function () {
    ...
    // Check if player is on ground to reset jump count
    if (this.player.body.touching.down) {
      this.jumpCount = 0;
    }

    // Check for jump key press (detect new press, not held down)
    const currentJumpKeyState = this.keyWAD.up.isDown || this.cursors.up.isDown || this.spaceKey.isDown;
    const jumpKeyJustPressed = currentJumpKeyState && !this.lastJumpKeyState;
    this.lastJumpKeyState = currentJumpKeyState;

    // Handle jumping with double jump
    if (jumpKeyJustPressed && this.jumpCount < this.maxJumps) {
